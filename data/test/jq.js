/**
 * @author: hwb
 * @date: 2021-09-24 16:30
 * @description：jq
 * @update: 2021-09-24 16:30
 */
/*!
...
return jQuery.Deferred(function (newDefer) {
                    ...
                    //mark Yao
                    xThen: function (doneHandle) {
                        var the = this;
                        //console.log(the)

                        return the.then(function (res, status) {
                            if (res) {
                                return res.flag == 1 ? doneHandle(res) : $.ajax.unDone(res, status, the._ajaxUrl, the._ajaxParams); //dtd.rejectWidth(this,[res,status]),dtd
                            } 
                                //why res empty?
                            
                        }, function (res, status) {
                            if (res) {
                                $.ajax.serverErrHandle(res, status);
                            } else {
                                //why res empty?
                            }
                        }).always(function () {
                            the._ajaxComplete && the._ajaxComplete();
                        });
                    },
...
return readyList.promise(obj);
    ...
    function Data() {
        this.expando = jQuery.expando + Data.uflag++;
    }
...
return adjusted;
    ...
    function on(elem, types, selector, data, fn, one) {
        ...
        if (typeof types === 'string' && ~types.indexOf('@')) {
            return on(elem, types.split('@')[0], types.split('@')[1], selector, data, fn);
        }
        ...
        if (typeof types === 'object') {
            ...
            for (type in types) {
                if (~type.indexOf('@')) {
                    on(elem, type.split('@')[0], type.split('@')[1], data, types[type], one);
                } else {
...
return this;
            ...
            //mark Yao
            jqXHR.xThen = function (doneHandle) {
                var the = this;
                var d = the.then(function (res, status) {
                    if (res) {
                        var query;

                        if (res.flag == 1) {
                            query = doneHandle(res);//console.info(q)
                            if (query && query.readyState) {
                                d._ajaxUrl = query._ajaxUrl;
                                d._ajaxParams = query._ajaxParams;
                                d._ajaxComplete = query._ajaxComplete;
                            }
                        } else {
                            $.ajax.unDone(res, status, s.url, s.data); //dtd.rejectWidth(this,[res,status]),dtd
                        }
...
return query;
                    ...
                    if (res) {
                        $.ajax.serverErrHandle(res, status, s.url);
                    } else {
                        //why res empty?
                    }
                }).always(function () {
...
return d;
            ...
            jqXHR._ajaxUrl = s.url;
            jqXHR._ajaxParams = s.data;
